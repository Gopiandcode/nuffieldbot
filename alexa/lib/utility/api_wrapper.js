...
function generate_available_intent(class_type, date, time, attributes, callback) {
    ...
    let booking_dates = date ? data_utilities.convertDateSlotValue(date) : {startDate: new Date(), endDate: new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 7)};
    let converted_class = data_utilities.convertAlexaSpeach(class_type);
    ...
    let options = auth.options_event_request;
    options.url = urls.url_retrieve_events(booking_dates, id);
    ...
    request(options, function(error, resp, body) {
        ...
        if(request_limit === 0) {
            let none_found = "Unfortunately there are no " + converted_class + " classes available during that time.";
            ...
            data._embedded.events.forEach(function(obj) {
                ...
                let second_options = auth.options_event_request;
...
function is_valid_available_class(class_type, datestring, timestring, attributes, callback) {
    ...
    let options = auth.options_event_request;
    options.url = urls.url_retrieve_events(date, id);
    options.headers['auth-token'] = attributes.auth_token;
